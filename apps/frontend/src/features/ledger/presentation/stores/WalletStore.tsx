...
export default class WalletStore {
  ...
  public async connectKeplr(): Promise<void> {
    ...
    try {
      ...
      sessionStorage.setItem(
        SESSION_STORAGE_WALLET_KEY,
        SessionStorageWalletOptions.KEPLR,
      );
  ...
  public async tryConnect(): Promise<void> {
    const sessionStorageWalletOptions = sessionStorage.getItem(
      SESSION_STORAGE_WALLET_KEY,
    );
  ...
  formatBalanceInCudosFraction(): string {
    return (
      this.balance
        ?.minus(this.balance.integerValue(BigNumber.ROUND_DOWN))
        .shiftedBy(4)
        .toFixed(0) ?? '0000'
    );
  }
