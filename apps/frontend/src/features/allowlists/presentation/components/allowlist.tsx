...
import { formatAddress, isMainnetInstance } from '../../../../core/utilities/ProjectUtils';
...
const Allowlist = ({ props }: { props: FetchedAllowlist }) => {
  ...
  console.log(props.end_date.toLocaleDateString())


  return (
    <Box id='allowlist' gap={8} sx={allowListStyles.holder}>
      <Box
        id='allowlistHolder'
        sx={allowListStyles.contentHolder}
      >

        <Box sx={{ position: 'relative' }}>
          <img style={{ borderRadius: '16px', width: '100%', maxHeight: '400px' }} src={props.banner_image} />
          <img style={{ backdropFilter: 'blur(16px)', border: "2px solid #F5FAFF", borderRadius: '16px', position: 'absolute', left: 40, bottom: -40, maxWidth: '224px', width: '25%' }} src={props.image} />
          <Box sx={{ padding: '48px', position: 'absolute', background: COLORS_DARK_THEME.PRIMARY_DARK_BLUE_80, right: 40, bottom: -80, borderRadius: '24px' }}>
            AASFASGFSA ASFAS ASGASG
          </Box>
        </Box>
        {"Booooo ".repeat(200)}
      </Box>
